...
|*reverse_relationships* (): `xref:BASE:foundation_types:structure_types.adoc#_list_class[List]<xref:BASE:base_types:identification_package.adoc#_locatable_ref_class[LOCATABLE_REF]>` +
 +
__Post_reverse_relationships_validity__: `Result /= Void implies (not Result.empty and then Result.for_all (item &#124; repository ("demographics").all_party_relationships.has_object (item) and then repository ("demographics").all_party_relationships.object (item).target = self))`
