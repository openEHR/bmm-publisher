[cols="^1,3,5"]
|===
h|*Class*
2+^h|*P_BMM_SINGLE_PROPERTY*

h|*Description*
2+a|Persistent form of `BMM_SINGLE_PROPERTY`.

h|*Inherit*
2+|`xref:LANG:bmm_persistence_package.adoc#_p_bmm_property_class[P_BMM_PROPERTY]`

h|*Attributes*
^h|*Signature*
^h|*Meaning*

h|*0..1*
|*type*: `xref:BASE:foundation_types:primitive_types.adoc#_string_class[String]`
a|If the type is a simple type, then this attribute will hold the type name. If the type is a container or generic, then type_ref will hold the type definition. The resulting type is generated in type_def.

h|*0..1*
|*type_ref*: `xref:LANG:bmm_persistence_package.adoc#_p_bmm_simple_type_class[P_BMM_SIMPLE_TYPE]`
a|Type definition of this property computed from `_type_` for later use in `_bmm_property_`.

h|*0..1 +
(redefined)*
|*bmm_property*: `xref:LANG:bmm:core_package.adoc#_bmm_property_class[BMM_PROPERTY]`
a|`BMM_PROPERTY` created by `_create_bmm_property_definition_`.

h|*Functions*
^h|*Signature*
^h|*Meaning*

h|*1..1*
|*type_def* (): `xref:LANG:bmm_persistence_package.adoc#_p_bmm_simple_type_class[P_BMM_SIMPLE_TYPE]`
a|Generate `_type_ref_` from `_type_` and save.
|===
