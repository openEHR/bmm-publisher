[cols="2,3", frame=none, grid=rows]
|===
2+^h|*LOCATABLE_REF*

2+a|Purpose Reference to a `LOCATABLE` instance inside the top-level content structure inside a `VERSION<T>`; the path attribute is applied to the object that `_VERSION.data_` points to.

2+|Inherits: xref:RM:support:identification_package.adoc#_object_ref_class[OBJECT_REF]

2+h|*Attributes*

|*id_namespace*: `xref:BASE:foundation_types:primitive_types.adoc#_string_class[String]` [1..1]
a|Namespace to which this identifier belongs in the local system context (and possibly in any other openEHR compliant environment) e.g.  terminology ,  demographic . These names are not yet standardised. Legal values for the namespace are:
    local  &#124;  unknown  &#124;  [a-zA-Z][a-zA-Z0-9_-:/&+?]&#42; +
_Inherited from xref:RM:support:identification_package.adoc#_object_ref_class[OBJECT_REF]_

|*type*: `xref:BASE:foundation_types:primitive_types.adoc#_string_class[String]` [1..1]
a|Name of the  class (concrete or abstract) of object to which this identifier type refers, e.g.  PARTY ,  PERSON ,  GUIDELINE  etc. These class names are from the relevant reference model. The type name  ANY  can be used to indicate that any type is accepted (e.g. if the type is unknown). +
_Inherited from xref:RM:support:identification_package.adoc#_object_ref_class[OBJECT_REF]_

|*id*: `xref:RM:support:identification_package.adoc#_uid_based_id_class[UID_BASED_ID]` [1..1]
a|The identifier of the Version.

|*path*: `xref:BASE:foundation_types:primitive_types.adoc#_string_class[String]` [0..1]
a|The path to an instance in question, as an absolute path with respect to the object found at `_VERSION.data_`. An empty path means that the object referred to by id being specified.

2+h|*Functions*

|*as_uri* (): `xref:BASE:foundation_types:primitive_types.adoc#_string_class[String]` [1..1]
a|A URI form of the reference, created by concatenating the following: ehr://  + id.value +  /  + path
|===
