...
h|*1..1*
|*persist_in_server*: `xref:BASE:foundation_types:primitive_types.adoc#_boolean_class[Boolean]`
a|If True, this Request is persisted in the server until further notice.

h|*0..1*
|*repeat_period*: `xref:RM:data_types:quantity_package.adoc#_dv_duration_class[DV_DURATION]`
a|Period for resending update Extracts in response to original Request.

h|*0..1*
|*trigger_events*: `xref:BASE:foundation_types:structure_types.adoc#_list_class[List]<xref:RM:data_types:text_package.adoc#_dv_coded_text_class[DV_CODED_TEXT]>`
...
h|*1..1*
|*update_method*: `xref:RM:data_types:text_package.adoc#_code_phrase_class[CODE_PHRASE]`
