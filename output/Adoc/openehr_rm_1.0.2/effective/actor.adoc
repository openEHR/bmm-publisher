...
|*languages*: `xref:BASE:foundation_types:structure_types.adoc#_list_class[List]<xref:RM:data_types:text_package.adoc#_dv_text_class[DV_TEXT]>` [1..1]
a|Languages which can be used to communicate with this actor, in preferred order of use (if known, else order irrelevant).

|*roles*: `xref:BASE:foundation_types:structure_types.adoc#_list_class[List]<xref:RM:support:identification_package.adoc#_party_ref_class[PARTY_REF]>` [1..1]
...
|*has_legal_identity* (): `xref:BASE:foundation_types:primitive_types.adoc#_boolean_class[Boolean]` [1..1]
